    ...
    Public Sub ComboBox_Journal_Of_Irradiation_View_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ComboBox_Journal_Of_Irradiation_View.SelectedIndexChanged
        Try
            ...
            If ComboBox_Journal_Of_Irradiation_View.SelectedItem = "За текущий год" Or ComboBox_Journal_Of_Irradiation_View.SelectedItem = "Current year" Or ComboBox_Journal_Of_Irradiation_View.SelectedItem = Nothing Then
                ...
                cmd.CommandText = "  SELECT        DISTINCT convert(varchar, a.DateTimeStart,23) +';' as dates    FROM Measurements  as a    where     's' not in     (        select            distinct ClientNumber        from Measurements        where             convert(varchar, a.DateTimeStart,23) =  convert(varchar, DateTimeStart,23) and            [Type] = 0    ) and        Year(a.DateTimeStart) = Year(GETDATE()) and        a.[Type] = 0 union    SELECT DISTINCT convert(varchar, m.DateTimeStart,23) + ';' + ClientNumber as dates    FROM Measurements as m     where     ClientNumber = 's' and        Year(m.DateTimeStart) = Year(GETDATE()) and        m.[Type] = 0 ORDER BY dates;"
            ...
            ElseIf ComboBox_Journal_Of_Irradiation_View.SelectedItem = "За все время" Or ComboBox_Journal_Of_Irradiation_View.SelectedItem = "All time" Then
                ...
                ListBox_SLI_Irradiation_Log_Date.DrawMode = DrawMode.OwnerDrawFixed
                ListBox_SLI_Irradiation_Log_Date.Items.Clear()
                cmd.CommandText = "SELECT        DISTINCT convert(varchar, a.DateTimeStart,23) +';' as dates    FROM Measurements  as a    where     's' not in     (        select            distinct ClientNumber        from Measurements        where             convert(varchar, a.DateTimeStart,23) =  convert(varchar, DateTimeStart,23) and            [Type] = 0    )  and        a.[Type] = 0 union    SELECT DISTINCT convert(varchar, m.DateTimeStart,23) + ';' + ClientNumber as dates    FROM Measurements as m     where     ClientNumber = 's'  and        m.[Type] = 0 ORDER BY dates;"
