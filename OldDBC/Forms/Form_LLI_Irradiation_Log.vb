        ...
        Try
            ...
            cmd.CommandText = "INSERT dbo.table_LLI_Irradiation_Log (Country_Code, Client_ID, Year, Sample_Set_ID, Sample_Set_Index, Sample_ID, Date_Start) " +
               ...
               DataGridView_LLI_Sample_Set.SelectedCells.Item(1).Value + "', '" +
                ...
                ListBox_Sample_ID.Text + $"', convert(datetime, '{MaskedTextBox_LLI_Irradiation_Log.Text}', 104))"
            ...
            'Table_LLI_Irradiation_Log_TableAdapter.Connection.ConnectionString = Form_Main.MyConnectionString
            'Table_LLI_Irradiation_Log_TableAdapter.Fill_LLI_Irradiation_Log(NAA_DB_EXPDataSet.table_LLI_Irradiation_Log, MaskedTextBox_LLI_Irradiation_Log.Text, Integer.Parse(TextBox_Download.Text))
                ...
                If IsDBNull(DataGridView_Table_SRM.Rows.Item(DataGridView_Table_SRM.CurrentCell.RowIndex).Cells.Item(i + 1).Value) = True Then
                    ...
                    cmd.CommandText = "INSERT dbo.table_LLI_Irradiation_Log (Country_Code, Client_ID, Year, Sample_Set_ID, Sample_Set_Index, Sample_ID, Date_Start) " +
                         " VALUES ('s', 's', 's', '" +
                          ...
                          DataGridView_Table_SRM.SelectedCells.Item(0).Value.ToString + $"', convert(datetime, '{MaskedTextBox_LLI_Irradiation_Log.Text}', 104))"
...
a:          cmd.CommandText = "DELETE FROM dbo.table_LLI_Irradiation_Log " +
    ...
    Private Sub Button_Samples_By_Detectors_Click(sender As System.Object, e As System.EventArgs) Handles Button_Samples_By_Detectors.Click
        Try
            ...
            Dim maxN = DataGridView_LLI_Irradiation_Log.Rows.Cast(Of DataGridViewRow).Select(Function(r) {If(Integer.TryParse(r.Cells("Container_Number").Value.ToString(), tmpint) = True, tmpint, 0)}(0)).ToList().Max()
            ' DataGridView_LLI_Irradiation_Log.Rows.Cast(Of Da ["Container_Number"]
            Dim _smpldetfrm As New Extensions.NewForms.SamplesToDetectors(maxN, Integer.Parse(TextBox_Download.Text), Form_Main.MyConnectionString)
            _smpldetfrm.Show()
        Catch ex As Exception
            If Form_Main.language = "Русский" Then
                MsgBox($"Операция была отменена!{vbCrLf}{ex.ToString()}", MsgBoxStyle.Critical, Me.Text)
            ElseIf Form_Main.language = "English" Then
                MsgBox($"Operation was cancelled!{vbCrLf}{ex.ToString()}", MsgBoxStyle.Critical, Me.Text)
            End If
            Exit Sub
        End Try
    End Sub
